...
describe("dedupeNetnsLabels", () => {
  it("suffixes duplicate labels with the inode tail", () => {
    const refs = dedupeNetnsLabels([
      { id: "4026533042", label: "systemd", nsPath: "/proc/100/ns/net" },
      { id: "4026533108", label: "systemd", nsPath: "/proc/200/ns/net" },
      { id: "4026533200", label: "bpflab", nsPath: "/var/run/netns/bpflab" },
    ]);
  ...
  it("leaves unique labels untouched", () => {
    const refs = dedupeNetnsLabels([
      { id: "1", label: "a", nsPath: "x" },
      { id: "2", label: "b", nsPath: "y" },
    ]);
