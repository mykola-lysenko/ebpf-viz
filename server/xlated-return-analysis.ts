...
const CALL_CLOBBERED_REGS: RegisterName[] = ["r0", "r1", "r2", "r3", "r4", "r5"];
...
function parseRegisterWrites(insn: XlatedInsn): RegisterWrite[] {
  ...
  const constMatch = body.match(/^([rw](?:10|[0-9]))\s*=\s*(-?(?:0x[0-9a-fA-F]+|\d+))\b/);
  if (constMatch) {
  ...
  const copyMatch = body.match(/^([rw](?:10|[0-9]))\s*=\s*([rw](?:10|[0-9]))\b/);
  if (copyMatch) {
  ...
  const unknownRegisterWrite = body.match(/^([rw](?:10|[0-9]))\s*(?:[-+*/%&|^]?=|[<>]{2}=|s[<>]{2}=)/);
...
function parseJump(insn: XlatedInsn, indexToPos: Map<number, number>): Pick<ParsedInsn, "jump" | "jumpTargetPos"> {
  ...
  return jumpTargetPos === undefined
    ? { jump }
    : { jump, jumpTargetPos };
}
...
function buildControlFlow(parsed: ParsedInsn[]): { predecessors: number[][]; reachable: Set<number> } {
...
function mergeResults(results: ResolveResult[]): ResolveResult {
  ...
  const unknown = results.find((result): result is Extract<ResolveResult, { kind: "unknown" }> => result.kind === "unknown");
  if (unknown) return unknown;

  const known = results as Array<Extract<ResolveResult, { kind: "known" }>>;
  const firstValue = known[0]?.value;
  if (firstValue === undefined || known.some(result => result.value !== firstValue)) {
    return { kind: "unknown", reason: "conflicting-values" };
  }

  const firstAssignmentIndex = known[0]?.assignmentInsn?.index;
  const assignmentInsn = firstAssignmentIndex !== undefined && known.every(result => result.assignmentInsn?.index === firstAssignmentIndex)
...
function writeForRegister(parsed: ParsedInsn, reg: RegisterName): RegisterWrite | undefined {
  return parsed.writes.find(write => write.reg === reg);
}

function resolveRegisterAtExit(parsed: ParsedInsn[], predecessors: number[][], exitPos: number): ResolveResult {
  ...
  const resolveBefore = (pos: number, reg: RegisterName): ResolveResult => {
    ...
    if (steps++ > MAX_RESOLUTION_STEPS) return { kind: "unknown", reason: "analysis-limit" };
    if (memo.has(key)) return memo.get(key)!;
    if (visiting.has(key)) return { kind: "unknown", reason: "analysis-limit" };

    visiting.add(key);
    const result = mergeResults(predecessors[pos].map(predPos => resolveAfter(predPos, reg)));
    visiting.delete(key);
    memo.set(key, result);
    return result;
  };
  ...
  const resolveAfter = (pos: number, reg: RegisterName): ResolveResult => {
    ...
    if (!write) {
      ...
      result = { kind: "known", value: write.value, assignmentInsn: write.insn };
    } else if (write.kind === "copy") {
      const source = resolveBefore(pos, write.source);
      result = source.kind === "known"
        ? { kind: "known", value: source.value, assignmentInsn: write.insn }
        : {
            kind: "unknown",
            reason: source.reason === "no-direct-assignment" ? "dynamic-assignment" : source.reason,
            assignmentInsn: write.insn,
          };
    } else {
      result = { kind: "unknown", reason: "dynamic-assignment", assignmentInsn: write.insn };
    }
...
function toKnownExit(exitInsn: XlatedInsn, resolved: Extract<ResolveResult, { kind: "known" }>): XlatedReturnExit {
  return {
...
function toUnknownExit(exitInsn: XlatedInsn, resolved: Extract<ResolveResult, { kind: "unknown" }>): XlatedReturnExit {
  return {
    exitIndex: exitInsn.index,
    exitDisasm: exitInsn.disasm,
    assignmentIndex: resolved.assignmentInsn?.index,
...
export function analyzeXlatedReturns(insns: XlatedInsn[]): XlatedReturnAnalysis {
  ...
  for (const parsedInsn of parsed) {
    ...
    const resolved = resolveRegisterAtExit(parsed, predecessors, parsedInsn.pos);
    if (resolved.kind === "known") {
      constantExits.push(toKnownExit(parsedInsn.insn, resolved));
      constantCounts.set(resolved.value, (constantCounts.get(resolved.value) ?? 0) + 1);
      continue;
    }
