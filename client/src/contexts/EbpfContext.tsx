...
export function EbpfProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadMapDumps = useCallback(async (file: File): Promise<{ loaded: number }> => {
    ...
    const obj = parsed as Record<string, unknown>;
    if (!obj._ebpfVizMapDumps) {
      throw new Error("Not a valid eBPF Viz map dump file.\nExpected a file generated by capture-snapshot.sh --dump-maps.");
    }
    const rawDumps = obj.mapDumps as Record<string, unknown[]>;
    if (!rawDumps || typeof rawDumps !== "object") {
